        ...
        <header className="text-gray-600 font-marvel bg-red-acid dark:bg-gray-500">
            <div className="container mx-auto flex flex-wrap p-2 flex-col md:flex-row items-center">
                ...
                <nav className="md:ml-auto flex flex-wrap items-center text-base justify-center">
                    {linksArr.map((e, i) =>
                        <NavLink to={e.path} key={i} className="cursor-pointer text-white text-xl hover:rounded-xl hover:bg-white p-5 mr-5 hover:text-black">{e.name}</NavLink>
                    )}
                </nav>
                <label htmlFor="red-toggle" className="inline-flex relative items-center mr-5 cursor-pointer">
                    <input type="checkbox" value="" id="red-toggle" className="sr-only peer" checked={mode} onChange={changeColorMode} />
